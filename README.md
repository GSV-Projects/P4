...
| readfill | Function that, reads from either a URL or filepath to a file in CSV format and inserts into a table, replacing empty values with surrounding column values | Table, URL/filepath | Table |
| replaceNA | Function that replaces all NA literals in a given column with a given value | Table, Column, Value | Array |
...
| filterall | Function that, given no parameter, will filter out any rows containing any NA values. If given some parameter that is either an expression or value, will filter out any rows that do not fulfill the expression or are not equal to the parameter | Table, **Parameter** | Table |
...
| frequency | Function that, given a column and some parameter, that can be an expression or a value, will count the frequency of entries in that column which fulfill the expression or are equal to the value | Table, Column, Parameter | Integer |
...
| max | Function that given a column of numbers returns the largest value | Table, Column | Float |
| span | Function that given a column of numbers returns the difference from the smallest and largest entry | Table, Column | Float |
